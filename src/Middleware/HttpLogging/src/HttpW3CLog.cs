...
{
    internal sealed class HttpW3CLog : IReadOnlyList<KeyValuePair<string, string?>>
    {
        private readonly List<KeyValuePair<string, string?>> _keyValues;
        private string? _cachedToString;

        internal static readonly Func<object, Exception?, string> Callback = (state, exception) => ((HttpW3CLog)state).ToString();

        public HttpW3CLog(List<KeyValuePair<string, string?>> keyValues)
        {
        ...
        public override string ToString()
        {
            if (_cachedToString == null)
            {
                ...
                var count = _keyValues.Count;
                builder.Append("Additional Fields:");
